...
class LocationBasedGenerator(nn.Module):
    '''
    output two dimensional coordinates
    '''
    ...
    def transform(self, x, theta): # x is defualt mask and theta is coordinates
        x = x[:, :3, :, :]
        grid = F.affine_grid(theta, x.size()) # (batch_size, 2, 3) a 2D affine transformation;  (batch_size, 3, 4) a 3D ffine transformation
        x = F.grid_sample(x, grid)
        return x

    def find_theta(self, x):
        '''
        x: original masks, (bs, nb masks, 3, 128, 128)
        
        theta from neural network

        '''
        xs = self.main(x)  # self.main is the neural network ; x = x.view(-1, 3, 128, 128)
        if xs.size(0) == 1:
    ...
    def return_sg(self, x, ob_names, if_return_trans_vec=False):
        """
        x: masks, (batch_size, nb masks, 3, 128, 128)
        ob_names: name for each mask
        return relationships
        """
        scene_img = torch.sum(x, dim=1) #sum over masks (bs, 3, 128, 128) --> scene image
        scene_img = torch.sum(scene_img, dim=1) # sum over three channels
        ...
        trans_vec = theta[:, :, [2, 5]]  # the x- coord and y-coord
        trans_vec[:, :, 0] *= -1  #negative the x-coord
        res = []

        for i in range(batch_size):
            sg = recon_sg(ob_names[i], trans_vec[i], nb_blocks_per_img[i])  # trans_vec[i] is coordinates
    ...
    def forward(self, x, x_default, weights, using_weights=True, only_pred=False):
        ...
        zeros = torch.zeros_like(pos_loss) # tensor
        hinge = nn.functional.mse_loss(x_default, x[:, :3, :, :]) # a loss value
...
if __name__ == '__main__':
    ...
    '''
    EDIT KE, original: tensor = to_tensor(pil).unsqueeze(0).cuda()
    '''
    
    tensor = to_tensor(pil).unsqueeze(0).cpu()
    for s in range(20):
        s/=10
        print(s)
