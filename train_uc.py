...
PARSER.add_argument("--dir", help="train directory",
                    default="/Users/pippala/Desktop/robotic/location-based-generative-master/photorealistic-blocksworld-master/blocks-5-3", type=str)
PARSER.add_argument("--eval_dir", help="2nd domain evaluation directory",
                    default="/Users/pippala/Desktop/robotic/location-based-generative-master/photorealistic-blocksworld-master/blocks-4-3", type=str)
PARSER.add_argument("--nb_samples", help="how many samples", default=3000, type=int)  # CHANGE # OF SAMPLES
...
def eval_f(model_, iter_, name_="1", device_="cuda"):
    ...
    for idx, train_batch in enumerate(iter_):
        ...
        if not vis:
            show2([
                torch.sum(start, dim=1)[:16].cpu(), #input scene image; start (n_sample*num_obj*3*128*128) just showing the firt 16 images
                torch.sum(start_pred, dim=1)[:16].detach().cpu(),  # the predictaed of start 
                start_pred.detach().cpu().view(-1, 3, 128, 128)[:16]+start.cpu().view(-1, 3, 128, 128)[:16], #overlapping of each object
...
def train():
    ...
    for epc in range(nb_epochs):
        ...
        for idx, train_batch in enumerate(train_iterator):
            optimizer.zero_grad()
            # Since (all_inp, targets, original_inp, weight_maps, sg, obj_names)
